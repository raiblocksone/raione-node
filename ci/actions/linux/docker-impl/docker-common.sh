...
if [[ "$GITHUB_WORKFLOW" != "Develop Branch Dockers Deploy" ]]; then
    docker_image_name="raiblocksone/raione${network_tag_suffix}"
fi
...
docker_build()
{
    ...
    if [[ "$GITHUB_WORKFLOW" != "Develop Branch Dockers Deploy" ]]; then
        ghcr_image_name="ghcr.io/${GITHUB_REPOSITORY}/raione${network_tag_suffix}"
...
docker_deploy()
{
    if [ -n "$DOCKER_PASSWORD" ]; then
        echo "$DOCKER_PASSWORD" | docker login -u raiblocksone --password-stdin
        if [[ "$GITHUB_WORKFLOW" = "Develop Branch Dockers Deploy" ]]; then
            "$scripts"/custom-timeout.sh 30 docker push "raiblocksone/raione-env:base"
            "$scripts"/custom-timeout.sh 30 docker push "raiblocksone/raione-env:gcc"
            "$scripts"/custom-timeout.sh 30 docker push "raiblocksone/raione-env:clang"
            echo "Deployed raione-env"
            exit 0
        else
            if [[ "$GITHUB_WORKFLOW" = "Live" ]]; then
                tags=$(docker images --format '{{.Repository}}:{{.Tag }}' | grep raiblocksone | grep -vE "env|ghcr.io|none|latest")
            else
                tags=$(docker images --format '{{.Repository}}:{{.Tag }}' | grep raiblocksone | grep -vE "env|ghcr.io|none")
            fi
            for a in $tags; do
